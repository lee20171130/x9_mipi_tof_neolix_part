...
#define LOG_TAG "neolixCubingJNILOG"
...
void getTofDepthDataCb(tofSensorAllData_t& tofSensorAllData)
{
	...
	case VIEW_VIDEO_MODE:
		...
		//将yuv转换为rgba
		DataTmp = yuvData;
		if (NULL != tofSensorAllData.pVideoData)
			DataTmp.data = tofSensorAllData.pVideoData;
		else {
			LOGE("%s:find nullprt tofSensorAllData.pVideoData!!!!!!\n", __func__);
			return;
		}
		break;
	default:;
  ...
  (JNIEnv *env, jclass jc, jobject dev_obj)
{
	...
	//注册jni层获得深度数据的回调函数
	sunny::registerJNIGetDepthCB(getTofDepthDataCb);
	...
	//存储深度数据的区域.如果获得体积,需要用到该数据帧(类型:深度)
